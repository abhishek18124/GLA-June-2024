...
int helper(vector<int>& nums, int n, int i) {
    ...
    return max(nums[i] + helper(nums, n, i + 2),
               helper(nums, n, i + 1));
...
int helperTopDown(vector<int>& nums, int n, int i, vector<int>& dp) {
    ...
    return dp[i] = max(nums[i] + helperTopDown(nums, n, i + 2, dp),
                       helperTopDown(nums, n, i + 1, dp));
...
int helperBottomUp(vector<int>& nums, int n) {
    ...
    for (int i = n - 1; i >= 0; i--) {
        ...
        // dp[i] = f(i) = find the maximum amount of money you can rob given a sequence of n-i houses [i...n-1] w/o alerting the police
